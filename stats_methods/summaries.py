...
def activity_with_without_feature(labelled_df: pd.DataFrame, out_dir: str):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(out_dir, 'presence_absence_means.jpg'), dpi=400)
    plt.close()
    plt.cla()
    plt.clf()
    plt.cla()
...
def compare_active_vs_inactive(active_df: pd.DataFrame, inactive_df: pd.DataFrame, out_dir: str):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(out_dir, 'activity_means.jpg'), dpi=400)
    plt.close()
    plt.cla()
    plt.clf()
...
def venn_diagram(df: pd.DataFrame, out_dir: str):
    ...
    plt.title('Activity')
    plt.savefig(os.path.join(out_dir, 'activity_venns.jpg'), dpi=400)
    # plt.show()
    plt.legend()
    plt.close()
    plt.cla()
    ...
    #     plt.tight_layout()
    #     # plt.show()
    #     plt.savefig(os.path.join(summary_output_dir, 'ethno_mean_comparison.jpg'))
    #     plt.close()
    plt.cla()
    plt.clf()
