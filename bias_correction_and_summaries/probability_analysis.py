...
def probabilities_of_selecting_active_species():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(_pa_output_path, 'P(sx)_labelled_distribution.jpg'), dpi=400)
    plt.close()
    plt.cla()
    plt.clf()
    ...
    h.set_xticks(np.linspace(0, 0.1, 11))
    plt.tight_layout()
    plt.savefig(os.path.join(_pa_output_path, 'P(sx)_labelled_distribution_small.jpg'), dpi=400)
    plt.close()
    plt.cla()
    plt.clf()
