...
def plot_corrected_means(logit_corrected_df, vars_to_compare: List[str], out_filename: str):
    from sklearn.preprocessing import MinMaxScaler

    corrected_df = logit_corrected_df[vars_to_compare].copy(deep=True)
    ...
    corrected_df = pd.DataFrame(s.transform(corrected_df),
                                columns=s.get_feature_names_out())

    labelled_means = labelled_traits.describe().loc[['mean']].values.tolist()[0]
    all_means = all_traits.describe().loc[['mean']].values.tolist()[0]

    logit_corrected_means = corrected_df.describe().loc[['mean']].values.tolist()[0]
